...
for category, codes in SIC_CATEGORIES.items():
    for code in codes:
        SIC_TO_CATEGORIES.setdefault(code, []).append(category)
...
def get_sic_categories(item: Dict[str, Any]) -> str:
    ...
    for code in item_sics:
        matched_categories.extend(SIC_TO_CATEGORIES.get(code, []))
    deduped_categories = []
    seen = set()
...
def upsert_company(conn: sqlite3.Connection, row: Dict[str, Any]) -> None:
    conn.execute(
        ...
        INSERT OR REPLACE INTO screened_companies (
            company_number, company_name, sic_code, sic_category, incorporation_date, company_type,
...
def validate_api_keys() -> List[str]:
    if "COMPANIES_HOUSE_API_KEY" not in st.secrets:
        raise ValueError("Missing COMPANIES_HOUSE_API_KEY in .streamlit/secrets.toml")
    key = str(st.secrets["COMPANIES_HOUSE_API_KEY"]).strip()
    if not key:
        raise ValueError("COMPANIES_HOUSE_API_KEY is empty")
    return [key]
...
def search_new_companies(client: CHClient, target_date: str) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    diagnostics = {
        ...
        "category_count": len(SIC_CATEGORIES),
    }
...
def apply_filters(
    ...
    if category_filter:
        ...
        filtered = filtered[filtered["Category"].astype(str).str.contains(category_pattern, case=False, na=False)].copy()
    if sic_search.strip():
...
def render_sidebar(default_date: date) -> Tuple[date, bool, List[str], str, str, bool, bool, List[str]]:
    ...
    return target_date, run, selected_signals, sic_search, company_name_search, only_flagged, shortlisted_only, category_filter
...
def main() -> None:
    ...
    with st.expander("Secrets format", expanded=False):
        st.code('COMPANIES_HOUSE_API_KEY = "your-api-key"', language="toml")
    ...
    target_date, run, selected_signals, sic_search, company_name_search, only_flagged, shortlisted_only, category_filter = render_sidebar(date.today())
    ...
    if run:
        ...
        with st.status("Running Companies House screening...", expanded=True) as status:
            ...
            st.write(f"SIC categories in use: {diagnostics['category_count']}")
            st.write(f"Unique SIC codes in use: {diagnostics['sic_count']}")
...
- Bonus scoring: companies get an extra star when a director appointment page shows identity verified by `ICON OFFICES LIMITED ACSP`
        ...
        st.write("Selected category filter:", ", ".join(category_filter) if category_filter else "None")
